...
```{r}
dec <- list(
  AL_KDR_HET_C = metadata(sce$AL_KDR_HET_C)$modelGeneVar,
  AL_KDR_HOM_D = metadata(sce$AL_KDR_HOM_D)$modelGeneVar
)
```
...
```{r}
ind <- c("AL_KDR_HET_C", "AL_KDR_HOM_D")
```
...
mnn$label <- factor(com$membership)
```

Tabulate number of cells from multiple batches within each cluster:

```{r}
tab <- table(Cluster = mnn$label, Batch = mnn$batch)
...
```{r}
ggcells(mnn, aes(corrected.1, corrected.2, colour = label)) + 
  geom_point(size = 1) + 
...
```{r}
ggcells(mnn, aes(TSNE.1, TSNE.2, colour = label)) +
  geom_point(size = 1) + 
  scale_colour_brewer(palette = "Set3") + 
...
```{r}
ggcells(mnn, aes(UMAP.1, UMAP.2, colour = label)) +
  geom_point(size = 1) +
  scale_colour_brewer(palette = "Set3") + 
